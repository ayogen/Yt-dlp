...
object EmbeddedExtractorEngine {
    ...
    @Deprecated("Use analyzeMediaCollection() for canonical extraction", ReplaceWith("analyzeMediaCollection(url)"))
    suspend fun analyzeUrl(url: String): Result<MediaMetadata> = withContext(Dispatchers.IO) {
        val colResult = analyzeMediaCollection(url)
        if (colResult.isSuccess) {
            Result.success(colResult.getOrThrow().toMediaMetadata())
        } else {
            Result.failure(colResult.exceptionOrNull() ?: Exception("Embedded extraction failed"))
        }
    }

    suspend fun analyzeMediaCollection(url: String): Result<MediaCollection> = withContext(Dispatchers.IO) {
        try {
            ...
            if (isDirectMedia) {
                return@withContext extractDirectMediaCollection(validatedUrl)
            }

            // Extract webpage HTML to obtain OpenGraph and meta tags without fake numbers
            val htmlContent = fetchWebpage(validatedUrl)
            val collection = parseWebpageMediaCollection(validatedUrl, host, htmlContent)
            AppLogger.i("EmbeddedExtractor", "Extracted web page collection for: ${collection.title}")
    ...
    private suspend fun extractDirectMediaCollection(url: String): Result<MediaCollection> {
        ...
        try {
            ...
            client.executeAsync(headRequest).use { response ->
        ...
        val kind = if (isAudio) MediaKind.AUDIO else MediaKind.VIDEO
        val singleItem = MediaItem(
            ...
            formats = formats,
            fileSize = contentLength,
            sizeProvenance = if (contentLength != null && contentLength > 0) SizeProvenance.EXACT else SizeProvenance.UNKNOWN,
            index = 0
        )
    ...
    private fun parseWebpageMediaCollection(url: String, host: String, html: String): MediaCollection {
        ...
        val kind = if (isPlaylist) MediaKind.PLAYLIST else MediaKind.VIDEO
        val singleItem = MediaItem(
            id = Math.abs(url.hashCode()).toString(),
            ...
            formats = formats,
            fileSize = null,
            sizeProvenance = SizeProvenance.UNKNOWN,
            index = 0
        )
    ...
    suspend fun downloadDirectStream(
        ...
        try {
            ...
            val response = client.executeAsync(requestBuilder.build())
